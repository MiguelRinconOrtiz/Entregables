...
int addLista (char nombre[20], int socket){
	if (miLista.num == 100)
		...
		pthread_mutex_lock(&mutex);// Modificamos la lista añadiendo un nuevo conectado
		strcpy(miLista.conectados[miLista.num].nombre, nombre);
		miLista.conectados[miLista.num].socket = socket;
		miLista.num++;
		pthread_mutex_unlock(&mutex);// desocupamos el canal 
		return 0;
	}
}
...
int delLista (char nombre[20]){
	...
	if (pos == -1)
		...
		int i;
		pthread_mutex_lock(&mutex);//eliminamos a alguien de la lista, se modifica
...
void *AtenderCliente(void *socket){
	...
	while (terminar ==0){
		...
		if (codigo ==0){ //petici?n de desconexi?n
			terminar=1;
			pthread_mutex_lock(&mutex);// eliminamos alguien de la lista, se desconecta
			int del = delLista(conectado);
			pthread_mutex_unlock(&mutex);//podemos interrumpir
			if (del == -1)
		...
		else if (codigo == 3){ //consulta 3 para añadir un usuario en lista de conectados
			p = strtok(NULL, "/");
			strcpy(nombre, p);
